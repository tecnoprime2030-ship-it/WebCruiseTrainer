...
function renderModelAnswer(q, isLocked) {
  ...
  if (isLocked) {
    // DON'T populate any content — show lock only
    overlay.style.display = 'flex';
    document.getElementById('modelAnswerBox').innerHTML = '';
    document.getElementById('kwChips').innerHTML = '';
    document.getElementById('fuQuestion').textContent = '';
    document.getElementById('fuAnswer').textContent   = '';
    document.getElementById('tipText').textContent    = '';
    if (toggleBtn) {
      toggleBtn.textContent  = '🔒 Premium — Unlock to see answers';
      toggleBtn.style.opacity = '0.6';
    }
    return; // stop here — nothing else rendered
  }

  // UNLOCKED — render everything
  overlay.style.display = 'none';
  if (toggleBtn) {
    toggleBtn.textContent  = '👁 Show Model Answer';
  ...
  box.innerHTML = `
    <div class="var-tabs">
      <button class="var-tab active" onclick="switchVariation('professional', this)">💼 Professional</button>
      <button class="var-tab" onclick="switchVariation('natural', this)">💬 Natural</button>
      <button class="var-tab" onclick="switchVariation('enthusiastic', this)">🔥 Enthusiastic</button>
    </div>
    <div class="var-content" id="varContent"></div>
    <button class="var-audio-btn" onclick="speakVariation()" id="varAudioBtn">🔊 Listen to this version</button>
  `;
  box.dataset.professional = vars.professional;
  box.dataset.natural      = vars.natural;
  box.dataset.enthusiastic = vars.enthusiastic;
  box.dataset.current      = 'professional';
  document.getElementById('varContent').textContent = vars.professional;
...
function toggleMic() {
  if (isListening) stopListening();
  else startListening(false); // false = fresh start, clears transcript
}

function continueSpeaking() {
  hideContinueButtons();
  startListening(true); // true = keep existing transcript
}

function scoreThisAnswer() {
  hideContinueButtons();
  if (userTranscript.trim().length > 3) scoreAnswer();
  else document.getElementById('answerStatus').textContent = 'Nothing to score. Press mic and try again.';
}

function hideContinueButtons() {
  const cb = document.getElementById('continueButtons');
  if (cb) cb.style.display = 'none';
}

function showContinueButtons() {
  let cb = document.getElementById('continueButtons');
  if (!cb) {
    cb = document.createElement('div');
    cb.id = 'continueButtons';
    cb.className = 'continue-btns';
    cb.innerHTML = `
      <p class="continue-label">Chrome stopped — what would you like to do?</p>
      <div class="continue-btn-row">
        <button class="action-btn btn-continue" onclick="continueSpeaking()">🎤 Continue speaking</button>
        <button class="action-btn btn-score" onclick="scoreThisAnswer()">✅ Score this answer</button>
      </div>
    `;
    document.getElementById('answerArea').appendChild(cb);
  }
  cb.style.display = 'block';
}

function startListening(keepTranscript) {
  stopSpeaking(); // stop TTS before mic

  if (!keepTranscript) userTranscript = '';

  if (/Mobi|Android/i.test(navigator.userAgent)) {
    document.getElementById('answerStatus').textContent =
      '📱 Mobile: speak clearly. Chrome desktop gives best results.';
  }

  const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
  if (!SR) {
    alert('Please use Google Chrome on desktop for voice features.');
    return;
  }

  recognition               = new SR();
  recognition.continuous    = false; // NO loop — Chrome stops once, no repeats
  recognition.interimResults = false; // NO interim — clean single result
  ...
  recognition.onstart = () => {
    ...
    document.getElementById('answerStatus').textContent    = '🎤 Speak now — press mic to stop';
  ...
  recognition.onerror = (event) => {
    ...
    if (event.error === 'no-speech') {
      // Silence detected — offer options if we have transcript
      if (userTranscript.trim().length > 3) {
        document.getElementById('answerStatus').textContent = 'Paused — continue or score your answer';
        showContinueButtons();
      } else {
        document.getElementById('answerStatus').textContent = 'No speech detected. Press mic and try again.';
      }
    } else {
  ...
  recognition.onend = () => {
    ...
    if (text.length > 3) {
      document.getElementById('answerStatus').textContent = 'Done — continue or score your answer';
      showContinueButtons();
    } else {
      document.getElementById('answerStatus').textContent = 'Nothing detected. Press mic and try again.';
    }
  };
...
function stopListening() {
  ...
  const text = userTranscript.trim();
  if (text.length > 3) {
    document.getElementById('answerStatus').textContent = 'Stopped — continue or score your answer';
    showContinueButtons();
  } else {
    document.getElementById('answerStatus').textContent = 'Nothing detected. Press mic and try again.';
  }
